...
class WQLinear(nn.Module):
    def __init__(self, w_bit, group_size, in_features, out_features, bias, dev):
        ...
        if w_bit not in [4, 2]:
            raise NotImplementedError("Only 4,2-bit are supported for now.")
        # if w_bit == 2:
        ...
        self.w_bit = w_bit
        self.offset = 0x0F if w_bit == 4 else 0x03
        self.group_size = group_size if group_size != -1 else in_features
        self.split_k_iters = pack_num = (32 // self.w_bit)
    ...
    def from_linear(cls, linear, w_bit, group_size, init_only=False, scales=None, zeros=None):
        ...
        #     awq_linear.qweight = trans(awq_linear.qweight)
        #     awq_linear.scales = trans(awq_linear.scales)
        #     awq_linear.qzeros = trans(awq_linear.qzeros)
        return awq_linear
    ...
    def forward(self, x):
        ...
        if inputs.shape[0] >= 1:
            if USE_TRITON:
                ...
                self.qzeros.T.contiguous(), 
                self.scales.T.contiguous(), 
                M=out_shape[-2], 
                N=out_shape[-1], 
                K=self.in_features, 
            ...
            out = awq_inference_engine.gemv_forward_cuda(inputs, self.qweight, self.scales, self.qzeros, self.w_bit, self.group_size)

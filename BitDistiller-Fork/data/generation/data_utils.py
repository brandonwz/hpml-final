...
def get_gen_dataset(dataset_name, max_sample=None, tokenizer=None):
    if dataset_name == "wikitext":
        ...
        return get_toxicchat_dataset(max_sample, tokenizer)
    else:
...
def get_toxicchat_dataset(max_sample, tokenizer):
    TOXICCHAT_CATEGORIES = {
                "S1":"sexual",
                "S2":"sexual/minors",
                "S3":"harassment",
                "S4":"self-harm",
    ...
    toxicchat_dataset = pd.read_csv("./toxic-chat/train.csv")
    user_prompts = toxicchat_dataset["user_input"].tolist()
    targets = toxicchat_dataset["toxicity"].tolist()
    sources = []
    for prompt in user_prompts:
        ...
        wrapped_prompt = tokenizer.apply_chat_template(
                                        wrapped_prompt_dict,
                                        tokenize=False,
                                        categories = TOXICCHAT_CATEGORIES
                                    )
        sources.append(wrapped_prompt)
